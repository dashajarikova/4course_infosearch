...
all_lemmas = fasttext_model.vocab
lemmas_vectors = np.zeros((len(all_lemmas), fasttext_model.vector_size))
for idx, lemma in enumerate(all_lemmas):
...
def sent_vectorizer(sent, model):
    ...
    for idx, lemma in enumerate(sent):
        if lemma in model.vocab:
            lemmas_vectors[idx] = model[lemma]=0)
    return sent_vector

data['question1'] = data['question1'].apply(lambda x: preproc(x))
data['question2'] = data['question2'].apply(lambda x: preproc(x)) #3-4min
data['q1_vector'] = data['question1'].apply(lambda x: sent_vectorizer(x, fasttext_model))
data['q2_vector'] = data['question2'].apply(lambda x: sent_vectorizer(x, fasttext_model))

from sklearn.metrics.pairwise import cosine_similarity
def cos_sim_func(vec_1, vec_2):
    vec_1 = vec_1.reshape(1, -1)
    vec_2 = vec_2.reshape(1, -1)
    vec_1 = np.nan_to_num(vec_1)
    vec_2 = np.nan_to_num(vec_2)
    cos_sim_val = cosine_similarity(vec_1, vec_2)
    return cos_sim_val

data['cos_sim'] = data.apply(lambda row: cos_sim_func(row.q1_vector, row.q2_vector), axis=1)
aver_cos_sim = data['cos_sim'].mean()
data['duplicate_predicted'] = data['cos_sim'].apply(lambda x: 1 if x > 0.5 else 0)

aver_cos_sim #0.64628356
